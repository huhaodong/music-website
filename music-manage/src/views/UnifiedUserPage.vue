<template>
  <div class="container">
    ...
    <el-table height="550px" border stripe :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="头像更新" width="90" align="center">
        <template v-slot="scope">
          <el-upload
...
import { HttpManager, SystemManager } from "@/api";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function getRoles() {
      try {
        const result = (await SystemManager.getAllRoles()) as ResponseBody;
        if (result.data) {
    ...
    async function submitUserForm() {
      ...
      try {
        if (isEdit.value) {
          const result = await SystemManager.updateUser(userForm) as ResponseBody;
          ElMessage.success(result.message || "更新成功");
        } else {
          const result = await SystemManager.addUser(userForm) as ResponseBody;
    ...
    async function deleteAll() {
      ...
      for (const item of multipleSelection.value) {
        await HttpManager.deleteUser(item.id) as ResponseBody;
      }
